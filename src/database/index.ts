...
export class BibliographyDB {
    ...
    constructor(app: BibliographyApp) {
        this.app = app
        this.db = {}
        this.cats = []
        this.sc = new BibliographyDBServerConnector(app.apiConnectors.bibliography)
    }
        ...
        return this.sc
            ...
            .then(
                ({bibCats, bibList, lastModified, numberOfEntries, userId}) => {
                    ...
                    let bibListItems: ServerBibItem[]
                    if (bibList) {
                        bibListItems = bibList
                        window.localStorage.setItem(
                            "biblist",
                            JSON.stringify(bibListItems)
                        )
                        window.localStorage.setItem(
                        ...
                        window.localStorage.setItem("owner_id", String(userId))
                        window.localStorage.setItem(
                            "version",
                            FW_LOCALSTORAGE_VERSION
                        )
                    } else {
                        bibListItems = JSON.parse(
                            window.localStorage.getItem("biblist") || "[]"
                        )
                    }
                    bibListItems.forEach(({id, bibDBEntry}) => {
     ...
     */
    deleteCategory(ids: number[]): Promise<number[]> {
        return this.sc.deleteCategory(ids).then(() => {
            const deletedPks = ids.slice()
            const deletedBibCats: BibCategory[] = []
    ...
    deleteBibEntries(ids: number[]): Promise<number[]> {
        activateWait()
        ids = ids.map(id => Number.parseInt(String(id)))
        return this.sc
